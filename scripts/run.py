...
from tqdm import tqdm
pyngp_path = 'out/build/x64-Debug'
...
def parse_args():
    ...
    parser.add_argument("--mode", default="", const="nerf", nargs="?", choices=[
                        "nerf", "sdf", "image", "volume"], help="Mode can be 'nerf', 'sdf', 'image' or 'volume'. Inferred from the scene if unspecified.")
    ...
    parser.add_argument("--screenshot_frames", nargs="*",
    ...
    parser.add_argument("--screenshot_spp", type=int, default=16,
                        help="Number of samples per pixel in screenshots.")

    parser.add_argument("--video_camera_path", default="",
    ...
    parser.add_argument("--video_output", type=str,
                        default="video.mp4", help="Filename of the output video.")
    ...
    parser.add_argument("--width", "--screenshot_w", type=int,
                        default=0, help="Resolution width of GUI and screenshots.")
    parser.add_argument("--height", "--screenshot_h", type=int,
                        default=0, help="Resolution height of GUI and screenshots.")
    ...
    parser.add_argument("--sharpen", default=0,
...
if __name__ == "__main__":
    ...
    args.mode = args.mode or mode_from_scene(
        args.scene) or mode_from_scene(args.load_snapshot)
    if not args.mode:
        raise ValueError(
        ...
        with tqdm(list(enumerate(test_transforms["frames"])), unit="images", desc=f"Rendering test frame") as t:
            for i, frame in t:
                ...
                if testbed.color_space == ngp.ColorSpace.SRGB and ref_image.shape[2] == 4:
                    ...
                    ref_image[..., :3] = np.divide(ref_image[..., :3], ref_image[..., 3:4], out=np.zeros_like(
                        ref_image[..., :3]), where=ref_image[..., 3:4] != 0)
                ...
                image = testbed.render(
                    ref_image.shape[1], ref_image.shape[0], spp, True)
    ...
    if ref_transforms:
        ...
        for idx in args.screenshot_frames:
            ...
            image = testbed.render(args.width or int(ref_transforms["w"]), args.height or int(
                ref_transforms["h"]), args.screenshot_spp, True)
    ...
    if args.video_camera_path:
        ...
        for i in tqdm(list(range(min(n_frames, n_frames+1))), unit="frames", desc=f"Rendering video"):
            ...
            frame = testbed.render(resolution[0], resolution[1], args.video_spp, True, float(
                i)/n_frames, float(i + 1)/n_frames, args.video_fps, shutter_fraction=0.5)
